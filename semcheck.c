...
__BOOLEAN insertParamIntoSymTable( struct SymTable *table, struct param_sem *params, int scope, int* _next_num )
{
			...
			if( parPtr->pType->isError == __TRUE ) { 
				...
				for( idPtr=(parPtr->idlist) ; idPtr!=0 ; idPtr=(idPtr->next) ) {
					if( verifyRedeclaration( table, idPtr->value, scope ) ==__FALSE ) { result = __TRUE;  }
						...
						newNode = createParamNode( idPtr->value, scope, parPtr->pType, *_next_num );
...
void insertFuncIntoSymTable( struct SymTable *table, const char *id, struct param_sem *params, struct PType *retType, int scope, FILE* java_file )
{
	if( verifyRedeclaration( table, id, scope ) == __FALSE ) { return; }
		...
		//code generate
		char java_cmd[256];
		strcpy(java_cmd, ".method public static ");
		strcat(java_cmd, id);
		strcat(java_cmd, "(");
		...
		if( params == 0 ) {
			...
			for( parPtr=params ; parPtr!=0 ; parPtr=(parPtr->next) ) {
				for( idPtr=(parPtr->idlist) ; idPtr!=0 ; idPtr=(idPtr->next) ) {
					...
					//code generate
					strcat(java_cmd, java_type(parPtr->pType) );
				}
			}
		}
		...
		//code generate
		strcat(java_cmd, ")");
		strcat(java_cmd, java_type(retType) );
		strcat(java_cmd, "\n");
		fwrite(java_cmd, sizeof(char), strlen(java_cmd), java_file);
		strcpy(java_cmd, ".limit stack 100\n");
		fwrite(java_cmd, sizeof(char), strlen(java_cmd), java_file);
		strcpy(java_cmd, ".limit locals 100\n");
		fwrite(java_cmd, sizeof(char), strlen(java_cmd), java_file);
	}
}
