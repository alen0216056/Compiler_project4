...
void printSymTable( struct SymTable *table, int __scope )
{
	...
	printf("Name                             Kind       Level      Type                           next_num Attribute                \n");
	printf("------------------------------------------------------------------------------------------------------------------------\n");
	int i;
	struct SymNode *ptr;
	for( i=0 ; i<HASHBUNCH ; i++ ) {
...
void load_var( struct SymTable *table, struct expr_sem *expr, int scope, char* fileName, FILE* java_file )
{
	...
	else
	{
		...
		else if( node->category==VARIABLE_t || node->category==PARAMETER_t || node->category==LOOPVAR_t )	//local
		{
			fprintf(java_file, "%sload %d\n", lower_java_type(node->type), node->next_num);
		}
		else
		{
		}
